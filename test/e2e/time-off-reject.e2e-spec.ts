...
describe('POST /requests/:id/reject', () => {
  ...
  it('returns 409 INVALID_TRANSITION with currentStatus when rejecting a request that is already approved', async () => {
    seedBalance('emp-reject-02', 'loc-BR', 'PTO', 10);
    const pending = await createPendingRequest({
      employeeId: 'emp-reject-02',
      clientRequestId: 'client-reject-02',
    });
  ...
  it('returns 404 REQUEST_NOT_FOUND for an unknown request id', async () => {
    ...
    const response = await request(ctx.app.getHttpServer())
      .post(`/requests/${unknownId}/reject`)
      .send();
